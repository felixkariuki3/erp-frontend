...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    await fetch(`${process.env.REACT_APP_API_URL}/api/email`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...formData, ...result })
    });

    await fetch(`${process.env.REACT_APP_API_URL}/api/log`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...formData, ...result })
    });
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-2xl bg-white shadow-md rounded-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700">Company Size</label><input name="companySize" value={formData.companySize} onChange={handleChange} placeholder="e.g. 100" className="mt-1 block w-full border border-gray-300 rounded-md p-2" /></div>
          <div><label className="block text-sm font-medium text-gray-700">Pain Points</label><input name="painPoints" value={formData.painPoints} onChange={handleChange} placeholder="e.g. Too costly, manual reporting" className="mt-1 block w-full border border-gray-300 rounded-md p-2" /></div>
          <div><label className="block text-sm font-medium text-gray-700">Modules Needed</label><input name="modules" value={formData.modules} onChange={handleChange} placeholder="e.g. Finance, CRM" className="mt-1 block w-full border border-gray-300 rounded-md p-2" /></div>
          <div>
          ...
          <div><label className="block text-sm font-medium text-gray-700">Budget</label><input name="budget" value={formData.budget} onChange={handleChange} placeholder="e.g. 20000" className="mt-1 block w-full border border-gray-300 rounded-md p-2" /></div>
          <div>
          ...
          <div><label className="block text-sm font-medium text-gray-700">Current System</label><input name="currentSystem" value={formData.currentSystem} onChange={handleChange} placeholder="e.g. QuickBooks, SAP" className="mt-1 block w-full border border-gray-300 rounded-md p-2" /></div>
          <div><label className="block text-sm font-medium text-gray-700">Email</label><input name="email" value={formData.email} onChange={handleChange} placeholder="e.g. user@example.com" className="mt-1 block w-full border border-gray-300 rounded-md p-2" /></div>
